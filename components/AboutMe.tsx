export default function AboutMe() {
  const IMAGES = [
    {
      path: "cycling.webp",
      text: "A super backlit photo of me and my bike during a century!",
    },
    {
      path: "trinity.webp",
      text: "Bat Lake on a recent backpacking trip to the Trinity Alps.",
    },
    {
      path: "tacoma.webp",
      text: "My partner and I visited Frank Herbert Park in Tacoma recently.",
    },
    {
      path: "boiz.webp",
      text: "A picture of my cats, Alucard & Loki, snuggling in their bed.",
    },
  ];

  return (
    <div className="bg-white w-full flex flex-col justify-center items-center gap-8 p-16">
      <h2 className="font-noto font-extrabold text-2xl">About Me</h2>
      <p className="font-noto">
        Aside from coding and web development, I keep myself busy with hiking,
        music, video games, and my lovely cats.
      </p>
      <div className="bg-white max-w-[900px] flex flex-row justify-center flex-wrap gap-8">
        {IMAGES.map((img) => (
          <div className="relative gap-4 z-10 flex flex-col min-w-[300px]">
            <img
              className="relative z-30 hover:z-10 rounded-lg shadow-brutal hover:scale-110"
              width={400}
              height={300}
              src={img.path}
              alt={img.text}
            />
            <div class="relative z-0 max-w-[400px] shadow-brutal bg-light-green p-4 rounded-lg">
              <p className="font-noto font-semibold text-black">{img.text}</p>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
